...
class PostDetailView(View):
    ...
    def get(self, request, post_id, post_slug):
        ...
        return render(request, 'home/detail.html',
                      {'post':self.post_instance, 'comments':comments, 'form':self.form_class, 'reply_form':self.form_class_reply, 'can_like':can_like})
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            new_comment.save()
            messages.success(request,'your comment submittedd successfully.','success')
...
class PostUpdateView(LoginRequiredMixin, View):
    ...
    def setup(self, request, *args, **kwargs):
        self.post_instace = get_object_or_404(Post,pk=kwargs['post_id'])
        return super().setup(request, *args , **kwargs)

    def dispatch(self, request, *args, **kwargs):
        post = self.post_instace
        if not post.user.id == request.user.id :
            messages.error(request, 'tou cant update this post', 'danger')
            return redirect('home:home')
